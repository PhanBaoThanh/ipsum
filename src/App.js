...
function App() {
  const [numberPara,setNumberPara] = useState(1)
  ...
  return (
    <div className="App">
      <h3 className='header'>TIRED OF BORING LOREM IPSUM?</h3>
      <div className='info'>
        <p>Paragraphs:</p>
        <input ref={inputRef} type='number' defaultValue={numberPara} className='input' />
        <button className='btn' onClick={() => setNumberPara(inputRef.current.value)}>generate</button>
      </div>
    {
      text.map((item,index) => {
        if(index < numberPara)
